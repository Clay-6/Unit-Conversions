...
mod tests {
    ...
    fn k_to_c() {
        let abs_zero = 0.0;
        assert_eq!(kelvin_to_celsius(abs_zero), -273.15);

        let room_temp = 294.15;
        assert_eq!(kelvin_to_celsius(room_temp), 21.0);
    }

    #[test]
    fn c_to_k() {
        let abs_zero = -273.15;
        assert_eq!(celsius_to_kelvin(abs_zero), 0.0);

        let room_temp = 21.0;
        assert_eq!(celsius_to_kelvin(room_temp), 294.15);
    }
}
